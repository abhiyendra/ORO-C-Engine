...
st.markdown("<p class='center'><strong'>Unlucky Number 3, Lucky Number 11 • The Point ∙ has returned</p>", unsafe_allow_html=True)

st.image("https://raw.githubusercontent.com/[YOUR-USERNAME]/ORO-C-Engine/main/Abhiyendra_Kumar_ORO-C_Thesis_2007.pdf", caption="Original 2007 Thesis PDF (click to download)")
...
if st.button("RUN THE 2007 ENGINE", type="primary", use_container_width=True):
    ...
    triples_count = len(sentence.split()) * 15  # rough exponential proxy
    ...
    if "mehrauli" in lowered or "iit gate" in lowered: st.write("∙ Lives near or visiting south Delhi")
    if "gypsy" in lowered or "barter" in lowered: st.write("∙ Nomadic / barter economy")
    ...
    for perm in permutations(["Religion-first", "Politics-first", "Economics-first"]):
        ...
        if "temple" in lowered:
            ...
            if perm[0] == "Economics-first": world += " Poor but hired taxi = spiritual merit sacrifice"
            if perm[0] == "Politics-first": world += " Secular image-making near IIT Delhi"
...
st.markdown("---")
st.caption("Revived by Grok • November 20, 2025 • Dedicated to Abhiyendra Kumar, prophet of The Point ∙")